#!/usr/bin/env bash
# Create Azure Function App (consumption, Node) and deploy integration-error-digest-job timer.
...
#   INTEGRATION_ERROR_DIGEST_JOB_STORAGE_NAME must be globally unique (3–24 lowercase alphanumeric)
#   INTEGRATION_ERROR_DIGEST_JOB_FUNC_APP_NAME default: allaboard365-integration-error-digest
#   INTEGRATION_ERROR_DIGEST_ENDPOINT_URL     full POST URL (optional if derived)
...
FUNC_APP_NAME="${INTEGRATION_ERROR_DIGEST_JOB_FUNC_APP_NAME:-allaboard365-integration-error-digest}"
...
if ! az functionapp show --name "$FUNC_APP_NAME" --resource-group "$RESOURCE_GROUP" &>/dev/null; then
  if [[ -z "${INTEGRATION_ERROR_DIGEST_JOB_STORAGE_NAME:-}" ]]; then
    SUF="$(openssl rand -hex 4)"
    INTEGRATION_ERROR_DIGEST_JOB_STORAGE_NAME="allaboardiedj${SUF}"
  fi
  echo "Storage account:    $INTEGRATION_ERROR_DIGEST_JOB_STORAGE_NAME"
  if ! az storage account show --name "$INTEGRATION_ERROR_DIGEST_JOB_STORAGE_NAME" --resource-group "$RESOURCE_GROUP" &>/dev/null; then
    echo "Creating storage account $INTEGRATION_ERROR_DIGEST_JOB_STORAGE_NAME..."
    az storage account create \
      --name "$INTEGRATION_ERROR_DIGEST_JOB_STORAGE_NAME" \
      --resource-group "$RESOURCE_GROUP" \
      --location "$LOCATION" \
      --sku Standard_LRS
  else
    echo "Storage account $INTEGRATION_ERROR_DIGEST_JOB_STORAGE_NAME already exists."
  fi
  echo "Creating Function App $FUNC_APP_NAME..."
  az functionapp create \
    --name "$FUNC_APP_NAME" \
    --storage-account "$INTEGRATION_ERROR_DIGEST_JOB_STORAGE_NAME" \
...
if [[ -z "${BACKEND_ORIGIN}" ]] && [[ -t 0 ]] && [[ -z "${CI:-}" ]] && [[ -z "${SKIP_BACKEND_PROMPT:-}" ]]; then
  echo ""
  echo "Derive integration-error-digest POST URL (Enter = prod ${DEFAULT_ALLABOARD365_BACKEND_ORIGIN})."
...
if [[ -n "${BACKEND_ORIGIN}" ]]; then
  _B="${BACKEND_ORIGIN%/}"
  if [[ -z "${INTEGRATION_ERROR_DIGEST_ENDPOINT_URL:-}" ]]; then
    INTEGRATION_ERROR_DIGEST_ENDPOINT_URL="${_B}/api/scheduled-jobs/integration-error-digest"
    echo "→ Derived INTEGRATION_ERROR_DIGEST_ENDPOINT_URL"
  fi
fi

SETTINGS=()
if [[ -n "${INTEGRATION_ERROR_DIGEST_ENDPOINT_URL:-}" ]]; then
  SETTINGS+=("INTEGRATION_ERROR_DIGEST_ENDPOINT_URL=$INTEGRATION_ERROR_DIGEST_ENDPOINT_URL")
fi
...
if [[ ${#SETTINGS[@]} -gt 0 ]]; then
  ...
  echo "    --settings INTEGRATION_ERROR_DIGEST_ENDPOINT_URL='https://YOUR-HOST/api/scheduled-jobs/integration-error-digest' \\"
