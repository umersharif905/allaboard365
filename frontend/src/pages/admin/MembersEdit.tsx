...
const MembersEdit: React.FC<Props> = ({
  ...
  useEffect(() => {
    if (member) {
      // Fix date formatting - handle null/undefined dates and invalid date strings
      let formattedDate = '';
      if (member.DateOfBirth) {
        try {
          // Handle both date strings and ISO dates
          const date = new Date(member.DateOfBirth);
          if (!isNaN(date.getTime())) {
            formattedDate = date.toISOString().split('T')[0];
          }
        } catch (e) {
          console.warn('Invalid date format for DateOfBirth:', member.DateOfBirth);
        }
      }
  ...
  const handleSaveEditMember = async () => {
    ...
    try {
      ...
      // Prepare the update data, handling empty date
      const updateData = {
        ...editFormData,
