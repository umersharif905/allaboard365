...
const LedgerTab = ({ shareRequestId, onChanged }: LedgerTabProps) => {
  const [txs, setTxs] = useState<ShareRequestTransaction[]>([]);
  const [bills, setBills] = useState<ShareRequestBill[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  const [showAdd, setShowAdd] = useState(false);
  // null = creating; a TransactionId = editing that transaction.
  ...
  const openEdit = (t: ShareRequestTransaction) => {
      ...
      try {
        const [txRes, billsRes] = await Promise.all([
          apiService.get<TransactionsResponse>(
            `/api/me/vendor/share-requests/${shareRequestId}/transactions`,
  ...
  const handleSave = async () => {
    ...
    // On edit, send null for cleared optional fields so they actually clear.
    const payload = {
    ...
    try {
      if (editingTxId) {
        await apiService.put(`/api/me/vendor/share-requests/${srId}/transactions/${editingTxId}`, payload);
      } else {
        await apiService.post(`/api/me/vendor/share-requests/${srId}/transactions`, payload);
      }
      setForm(emptyForm);
      setShowAdd(false);
      setEditingTxId(null);
  ...
  const handleDelete = async (txId: string) => {
    ...
    const srId = shareRequestId;
    try {
      await apiService.delete(`/api/me/vendor/share-requests/${srId}/transactions/${txId}`);
      await load();
      onChanged?.();
    } catch (err) {
    ...
    <div className="p-4 sm:p-6 space-y-4">
      ...
      {loading ? (
        ...
        <>
                ...
                <tbody className="divide-y divide-gray-200 bg-white">
                  {txs.map((t) => (
                    <tr key={t.TransactionId}>
                      ...
                      <Td>
                        <span>{transactionTypeLabel(t.TransactionType)}</span>
                        {t.Description && (
          ...
          {/* Ledger breakdown — only the per-category figures that the top
              summary cards (Billed / Saved / Member paid / Reimbursed / Balance)
              do NOT already show. Billed, Balance, Reimbursed and the bill-derived
              Paid live up top; here we decompose Saved (Discounts + Financial aid)
              and Member paid (UA + Member payments), plus Provider payments. */}
          <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-5 gap-3 bg-gray-50 border border-gray-200 rounded-lg p-4">
            <Stat label="Member paid — UA" value={fmtCurrency(totalsByType['UA Payment'] ?? 0)} />
            <Stat label="Member paid — other" value={fmtCurrency(totalsByType['Member Payment'] ?? 0)} />
            {/* Discounts span the new single 'Discount' type plus legacy strings. */}
            <Stat label="Discounts" value={fmtCurrency(
              (totalsByType['Discount'] ?? 0) +
              (totalsByType['Discount from Provider'] ?? 0) +
              (totalsByType['Discount from Emry RBP'] ?? 0) +
              (totalsByType['Negotiation'] ?? 0)
            )} />
            <Stat label="Financial aid" value={fmtCurrency(
              (totalsByType['Financial Aid'] ?? 0) +
              (totalsByType['Discount from Emry FA'] ?? 0)
            )} />
            <Stat label="Provider payments" value={fmtCurrency(totalsByType['Payment to Provider'] ?? 0)} />
            {/* UA Reduction is a rare non-cash waiver; only surface it when used
                so it never silently disappears from the breakdown. */}
            {(totalsByType['UA Reduction'] ?? 0) !== 0 && (
              <Stat label="UA reduction" value={fmtCurrency(totalsByType['UA Reduction'] ?? 0)} />
            )}
          </div>
        </>
      )}
...
const TxModal = ({ form, editing, bills, saving, onChange, onSave, onClose }: TxModalProps) => {
  ...
  // Derive the displayed "new total" from the stored discount amount.
  const newTotalInput =
  ...
  >
    <div className="w-full max-w-xl bg-white rounded-lg shadow-xl p-4 space-y-3 max-h-[90vh] overflow-y-auto">
      <div className="flex items-center justify-between">
        <h3 id="add-tx-title" className="text-base font-semibold text-gray-900">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <Field label="Bill (optional)" className="col-span-2">
          <select
            value={form.billId}
            onChange={(e) => {
              // Changing the bill invalidates a discount-by-new-total amount
              // (it was derived from the previous bill's total). Clear it so the
              // stored discount never silently desyncs from what was typed.
        ...
        <Field label="Type *">
          <select
            ...
            onChange={(e) => {
              // Switching into/out of Discount changes how the amount field is
              // interpreted (discount vs. new-total). Clear the amount on the
              // switch so a stale value isn't reinterpreted under the new mode.
        ...
        {discountByTotal ? (
          // Discount on a specific bill: let the user type the resulting bill
          // total; we store the discount amount (original − new total).
