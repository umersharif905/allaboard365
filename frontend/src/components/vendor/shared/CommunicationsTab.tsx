...
interface MessageHistoryDetails extends MessageHistoryItem {
  body?: string;
}
...
interface DetailsResponse {
  success: boolean;
  data?: MessageHistoryDetails;
  message?: string;
}

interface CommunicationsTabProps {
  /** Base path for vendor communications endpoints (no trailing slash). */
  basePath: string;
}
...
const CommunicationsTab = ({ basePath }: CommunicationsTabProps) => {
  ...
  const fetchHistory = useCallback(async (p: number, cancelled: { value: boolean }) => {
    ...
    setError(null);
    try {
      const res = await apiService.get<ListResponse>(`${basePath}?page=${p}&limit=${LIMIT}`);
  ...
  const handleView = async (item: MessageHistoryItem) => {
    setViewingId(item.historyId);
    try {
      const res = await apiService.get<DetailsResponse>(`${basePath}/${item.historyId}`);
      ...
      if (item.messageType === 'Email') {
        ...
        win.document.write(
          `<pre style="font-family:sans-serif;padding:20px;white-space:pre-wrap">${body}</pre>`
        );
      }
      win.document.close();
    } finally {
    ...
    <div className="p-6">
      <div className="flex items-start gap-3 mb-4">
        ...
        <div>
          ...
          <p className="text-sm text-gray-600 mt-0.5">Emails and texts sent to this member.</p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <p className="text-sm text-gray-500">No communications on record for this member.</p>
      ) : (
        <>
