// CaseBillsTab — editable bills for a Case. Copied from share-requests/tabs/
// BillsTab and pointed at the /cases/:id endpoints (duplication is intentional;
// see docs/billing-rework/case-finances-design.md).
...
import CaseProviderPicker from '../CaseProviderPicker';

interface CaseBillsTabProps {
  caseId: string;
...
const CaseBillsTab = ({ caseId, onChanged }: CaseBillsTabProps) => {
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingBillId) {
        await apiService.put(`/api/me/vendor/cases/${caseId}/bills/${editingBillId}`, payload);
      } else {
        await apiService.post(`/api/me/vendor/cases/${caseId}/bills`, payload);
      }
      setForm(emptyForm);
      setShowAdd(false);
      setEditingBillId(null);
  ...
  const handleDelete = async (billId: string) => {
    ...
    try {
      await apiService.delete(`/api/me/vendor/cases/${caseId}/bills/${billId}`);
      await load();
      onChanged?.();
    } catch (err) {
...
const BillModal = ({
  ...
  <div
    role="dialog"
    aria-modal="true"
    aria-labelledby="add-case-bill-title"
  ...
  >
    <div className="w-full max-w-xl bg-white rounded-lg shadow-xl p-4 space-y-3 max-h-[90vh] overflow-y-auto">
      <div className="flex items-center justify-between">
        <h3 id="add-case-bill-title" className="text-base font-semibold text-gray-900">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        <Field label="Type">
          <select
            value={form.billType}
            onChange={(e) => onChange({ ...form, billType: e.target.value as CaseBillType })}
            className="w-full px-2 py-1.5 text-sm border border-gray-300 rounded"
          >
            {CASE_BILL_TYPES.map((t) => (
