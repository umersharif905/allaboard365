#!/usr/bin/env bash
# Deploy website-form-digest-job timer to an existing Azure Function App (zip + remote build).
...
if ! az functionapp show --name "$FUNC_APP_NAME" --resource-group "$RESOURCE_GROUP" &>/dev/null; then
  ...
  echo "   Create it first with create-and-deploy.sh, or set RESOURCE_GROUP / WEBSITE_FORM_DIGEST_JOB_FUNC_APP_NAME."
  exit 1
fi

echo "Deploying via zip (Azure CLI; Oryx builds on the Function App)..."
ZIP="/tmp/allaboard-website-form-digest-job-deploy-$$.zip"
rm -f "$ZIP"
(
  cd "$SCRIPT_DIR"
  zip -rq "$ZIP" . -x "node_modules/*" -x ".git/*" -x "local.settings.json"
)
...
echo "Timer: WebsiteFormDigest (Mondays at 13:00 UTC)."
