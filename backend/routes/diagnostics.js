...
router.get('/afd-endpoint', async (_req, res) => {
  try {
    ...
    res.json({
      ...
      data: {
        ...
        endpointName: AZURE_CONFIG.endpointName,
        endpointHostName: endpoint?.properties?.hostName || null,
        provisioningState: endpoint?.properties?.provisioningState || null,
        enabledState: endpoint?.properties?.enabledState || null
      }
    });
  } catch (error) {
    res.status(500).json({
      success: false,
      message: error.message || 'Failed to load Azure Front Door endpoint information'
    });
  }
});
 ...
 * Get current CORS configuration including custom domains
 * Requires authentication (SysAdmin only for security)
 */
router.get('/cors', authenticate, authorize(['SysAdmin']), async (_req, res) => {
  try {
    const { getCustomDomains } = require('../middleware/cors');
    ...
    res.status(500).json({
      success: false,
      message: error.message || 'Failed to load CORS configuration'
    });
  }
});

/**
 * POST /api/diagnostics/cors/refresh
 * Manually refresh the CORS custom domains cache
 * Requires authentication (SysAdmin only for security)
 */
router.post('/cors/refresh', authenticate, authorize(['SysAdmin']), async (_req, res) => {
  try {
    const { refreshCustomDomainsCache } = require('../middleware/cors');
    const customDomains = await refreshCustomDomainsCache();
    ...
    res.status(500).json({
      success: false,
      message: error.message || 'Failed to refresh CORS cache'
    });
  }
});
 ...
 */
router.get('/afd/route-config', authenticate, authorize(['SysAdmin']), async (req, res) => {
  try {
