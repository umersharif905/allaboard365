...
// Explicit tenant context (must run before handlers; parent router also applies this).
router.use(requireTenantAccess);
router.use(authorize(['TenantAdmin', 'SysAdmin']));

// In-progress drafts admin tab (shared with the vendor surface). TenantAdmin/
// SysAdmin (router-level) may list and delete.
registerDraftAdminRoutes(router);
...
/**
 * GET /api/me/tenant-admin/public-forms/templates
 */
...
/**
 * POST /api/me/tenant-admin/public-forms/templates
 * Body: { title?: string } — creates a new draft form (empty fields); publish after editing.
 */
...
/**
 * POST /api/me/tenant-admin/public-forms/templates/:formTemplateId/duplicate
 * Copies a template (all settings + latest definition) as a new unpublished
 * draft with " (Copy)" appended to the title.
 */
router.post('/templates/:formTemplateId/duplicate', async (req, res) => {
    try {
...
/**
 * DELETE /api/me/tenant-admin/public-forms/templates/:formTemplateId
 * Only allowed when the form has no submissions (FK-safe).
 */
router.delete('/templates/:formTemplateId', async (req, res) => {
    try {
        const { formTemplateId } = req.params;
        if (!uuidRe.test(formTemplateId)) {
            return res.status(400).json({ success: false, message: 'Invalid id' });
...
/**
 * GET /api/me/tenant-admin/public-forms/templates/:formTemplateId
 */
...
/**
 * GET /api/me/tenant-admin/public-forms/resolver-tenant-options?vendorId=<guid>
 * The tenants a vendor serves — pick-list for a form's "resolve members across
 * tenants" allow-list (ResolverTenantIds). Admin-only (router-level guard).
 */
...
/**
 * GET /api/me/tenant-admin/public-forms/templates/:formTemplateId/versions/:versionNumber
 */
...
/**
 * PATCH /api/me/tenant-admin/public-forms/templates/:formTemplateId
 */
router.patch('/templates/:formTemplateId', async (req, res) => {
    try {
        const { formTemplateId } = req.params;
        if (!uuidRe.test(formTemplateId)) {
            return res.status(400).json({ success: false, message: 'Invalid id' });
...
/**
 * POST /api/me/tenant-admin/public-forms/templates/:formTemplateId/header-image
 * multipart field name: file — JPEG, PNG, GIF, or WebP, max 2MB
 */
router.post(
...
/**
 * POST /api/me/tenant-admin/public-forms/templates/:formTemplateId/versions
 */
router.post('/templates/:formTemplateId/versions', async (req, res) => {
    try {
        const { formTemplateId } = req.params;
        if (!uuidRe.test(formTemplateId)) {
            return res.status(400).json({ success: false, message: 'Invalid id' });
...
/**
 * GET /api/me/tenant-admin/public-forms/templates/:formTemplateId/preview-payload
 * Care-team preview of a form template. Returns the published version's
 * definition when published; the latest draft otherwise (with a flag so the
 * UI can mark the preview as draft-only).
 */
...
/**
 * POST /api/me/tenant-admin/public-forms/templates/:formTemplateId/publish
 */
router.post('/templates/:formTemplateId/publish', async (req, res) => {
    try {
        const { formTemplateId } = req.params;
...
/**
 * GET /api/me/tenant-admin/public-forms/submissions/export
 * CSV export: same filters as list; max rows capped (default 5000, max 25000).
 */
...
/**
 * GET /api/me/tenant-admin/public-forms/submissions
 */
...
/**
 * GET /api/me/tenant-admin/public-forms/submissions/:submissionId/submission-pdf
 * Regenerates the submission PDF on the server (same renderer as submit-time blob). Use this for
 * downloads so tenants always get the full PDF even when no blob was stored or SAS fetch fails.
 */
...
/**
 * GET /api/me/tenant-admin/public-forms/submissions/:submissionId/submission-pdf-complete
 * Same as submission-pdf but ignores includeInPdf flags — every field appears.
 */
...
/**
 * GET /api/me/tenant-admin/public-forms/submissions/:submissionId
 */
        ...
        const filesWithSas = await Promise.all((files || []).map(async (f) => {
            ...
            if (next.BlobUrl && isBlobUrl(next.BlobUrl)) {
                try {
                    ...
                    console.warn('public-forms admin submission file SAS failed', e.message);
                }
            }
            return next;
        }));
        // Include field metadata (label, type) so the viewer can render
        // user-friendly labels instead of raw payload keys.
        let fieldMeta = [];
        try {
            ...
            console.warn('tenant-admin submission: field metadata unavailable', metaErr.message);
        }
...
/**
 * POST /api/me/tenant-admin/public-forms/submissions/:submissionId/send-summary-email
 * Body: { toEmail: string } — sends a read-only HTML summary (tenant-scoped).
 */
...
/**
 * GET /api/me/tenant-admin/public-forms/submissions/:submissionId/routing-notification-defaults
 * Returns the email addresses that would receive a routing notification right now if no override
 * recipients were supplied. The UI uses this to pre-populate the editable recipients field on the
 * "Queue routing notifications" panel.
 */
router.get(
...
/**
 * POST /api/me/tenant-admin/public-forms/submissions/:submissionId/queue-routing-notifications
 * Queues the same tenant routing emails as submit time (oe.MessageQueue). Issues a fresh anonymous
 * link (previous public links for this submission stop working).
 * Body (optional):
 *   - { additionalEmails?: string } comma-/semicolon-separated, or
 *     { additionalEmailsList?: string[] } explicit list of addresses.
 *   - { replaceDefaults?: boolean } when true, send ONLY to the supplied addresses and skip the
 *     template NotifyEmails / tenant ContactEmail / env fallback chain.
 *   - { linkBaseOverride?: string } absolute origin (e.g. http://localhost:5173) to embed as
 *     the link base in the email. Validated against an allow-list (localhost, the tenant's
 *     CustomDomain, or the configured default app host); ignored when not allowed.
 */
router.post(
...
/**
 * PATCH /api/me/tenant-admin/public-forms/submissions/:submissionId/linkage
 * Retroactive linkage of a submission to a share request or case
 * (forms-page redesign Slice D.1). Mutually exclusive: both can't be
 * non-null. Both null clears the linkage.
 */
...
/**
 * POST /api/me/tenant-admin/public-forms/submissions/:submissionId/resolve-member
 */
...
/**
 * POST /api/me/tenant-admin/public-forms/submissions/:submissionId/set-member
 */
...
/**
 * POST /api/me/tenant-admin/public-forms/submissions/:submissionId/retry-link
 */
...
// Mirror of vendor invitation endpoints. See backend/routes/me/vendor/public-forms.js
...
/**
 * POST /api/me/tenant-admin/public-forms/templates/:formTemplateId/send-anonymous-link
 * Email the template's public anonymous URL to a recipient — no invitation
 * row, no token, no member binding (B-012). Audit lives only in SendGrid
 * logs for now.
 */
...
/**
 * POST /api/me/tenant-admin/public-forms/invitations/:invitationId/renew
 * Revoke the old invitation and issue a new one with the same template /
 * member / mode / linkage / recipient. The original recipient link
 * returns 410 immediately. Body: { deliveryMethod: 'email' | 'copy' }.
 * Returns { invitationId, url, expiresAt }.
 *
...
/**
 * PATCH /api/me/tenant-admin/public-forms/invitations/:invitationId
