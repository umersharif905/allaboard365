/**
 * One-shot runner for sql-changes/2026-05-19-share-request-types.sql.
 * Uses the same DB connection as the app (reads backend/.env).
 *
 * Usage (from host): sudo docker exec allaboard365-2-backend \
 *   node /app/backend/scripts/run-share-request-types-migration.js
 *
 * Idempotent — the migration itself wraps each step in IF NOT EXISTS guards,
 * the seed uses NOT EXISTS, and the backfill only touches RequestTypeId IS NULL.
 */
...
const sqlFile = path.join(
    ...
    '..',
    'sql-changes',
    '2026-05-19-share-request-types.sql'
);
