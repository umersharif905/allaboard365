#!/usr/bin/env bash
# Deploy ledger-reconcile-job timer to an existing Azure Function App (zip + remote build).
...
if ! az functionapp show --name "$FUNC_APP_NAME" --resource-group "$RESOURCE_GROUP" &>/dev/null; then
  ...
  echo "   Create it first with create-and-deploy.sh, or set RESOURCE_GROUP / LEDGER_RECONCILE_JOB_FUNC_APP_NAME."
  exit 1
fi

echo "Deploying via zip (Azure CLI; Oryx builds on the Function App)..."
ZIP="/tmp/allaboard-ledger-reconcile-job-deploy-$$.zip"
rm -f "$ZIP"
(
  cd "$SCRIPT_DIR"
...
echo "Timer: LedgerReconcile (Sun 06:00 UTC) — weekly DIME ledger reconcile (report-only)."
