#!/usr/bin/env bash
# Deploy vendor-jobs timer to an existing Azure Function App (zip + remote build).
...
if ! az functionapp show --name "$FUNC_APP_NAME" --resource-group "$RESOURCE_GROUP" &>/dev/null; then
  ...
  echo "   Create it first with create-and-deploy.sh, or set RESOURCE_GROUP / VENDOR_JOBS_FUNC_APP_NAME."
  exit 1
fi

echo "Deploying via zip (Azure CLI; Oryx builds on the Function App)..."
ZIP="/tmp/allaboard-vendor-jobs-deploy-$$.zip"
rm -f "$ZIP"
(
  cd "$SCRIPT_DIR"
...
echo "Timer: VendorExportScheduler (every 5 minutes UTC)."
